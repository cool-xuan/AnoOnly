...
def sampler_pairs(X_train_tensor, y_train, epoch, batch_num, batch_size, s_a_a, s_a_u, s_u_u):
    ...
    for i in range(batch_num):  # i.e., drop_last = True
        ...
        for j in range(6):
            ...
            if j == 3:
                index_sub = np.random.choice(index_u, batch_size // 4, replace=False)  # unlabel部分可以变为False
                index.append(list(index_sub))

            if j > 3:
                index_sub = np.random.choice(index_u, batch_size // 2, replace=False)  # unlabel部分可以变为False
                index.append(list(index_sub))
