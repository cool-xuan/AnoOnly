...
class PReNet():
    def __init__(self, seed:int, model_name='PReNet', anomaly_only=False, epochs:int=50, batch_num:int=20, batch_size:int=512,
    ...
    def fit(self, X_train, y_train, ratio=None):
        ...
        self.X_train_tensor = torch.from_numpy(X_train).float().to(self.device)  # testing set
        ...
        fit(X_train_tensor=self.X_train_tensor, y_train=y_train, model=self.model, optimizer=optimizer,
            ...
            s_a_a=self.s_a_a, s_a_u=self.s_a_u, s_u_u=self.s_u_u, device=self.device, anomaly_only=self.anomaly_only, scheduler=scheduler)
