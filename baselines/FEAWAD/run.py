...
from keras.layers import Input, Dense, Subtract,concatenate,Lambda,Reshape, BatchNormalization
...
class FEAWAD():
    def __init__(self, seed, model_name='FEAWAD', save_suffix='test', anomaly_only=False):
        self.utils = Utils()
        ...
        self.save_suffix = save_suffix + '_anoOnly' if self.anomaly_only else save_suffix
    ...
    def fit(self, X_train, y_train, ratio=None):
        ...
        AEmodel.fit_generator(self.auto_encoder_batch_generator_sup(X_train, inlier_indices, self.args.batch_size, self.args.nb_batch, rng),
                                         steps_per_epoch=self.args.nb_batch, epochs=100, callbacks=[ae_checkpointer], verbose=0)
