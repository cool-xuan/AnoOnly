...
class DataGenerator():
    def __init__(self, seed:int=42, dataset:str=None, test_size:float=0.3,
                 generate_duplicates=True, n_samples_threshold=1000, backbone='small'):
        '''
    ...
    def generator(self, X=None, y=None, minmax=True,
                  la=None, norla=1., at_least_one_labeled=False,
                  ...
                  description_visible=True, normal_clean=False, backbone='small'):
        '''
        ...
        if self.dataset is None:
            ...
            if self.dataset in self.dataset_list_classical:
                ...
                feat_dir = 'CV_by_ResNet18' if backbone == 'small' else 'CV_by_ViT'
                data = np.load(os.path.join('datasets', feat_dir, self.dataset + '.npz'), allow_pickle=True)
            elif self.dataset in self.dataset_list_nlp:
                feat_dir = 'NLP_by_BERT' if backbone == 'small' else 'NLP_by_RoBERTa'
                data = np.load(os.path.join('datasets', feat_dir, self.dataset + '.npz'), allow_pickle=True)
            else:
        ...
        idx_normal = idx_normal[:ceil(len(idx_normal) * norla)]
        ...
        # print("\033[1;34;42m The number of anomalies: {}\033[0m".format(len(idx_labeled_anomaly)))
        ...
        y_train = y_train[np.append(idx_unlabeled, idx_labeled_anomaly)]
        X_train = X_train[np.append(idx_unlabeled, idx_labeled_anomaly)]
        print("Number of labeled anomalies: {}".format(len(idx_labeled_anomaly)))
        # y_train[idx_unlabeled] = 0
        # y_train[idx_labeled_anomaly] = 1
