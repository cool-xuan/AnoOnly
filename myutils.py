...
class Utils():
    ...
    def data_description(self, X, y):
        ...
        des_dict['Anomalies'] = sum(y != 0)
        des_dict['Anomalies Ratio(%)'] = round((sum(y != 0) / len(y)) * 100, 2)
    ...
    def metric(self, y_true, y_score, pos_label=1):
        metrics = {}
        metrics['aucroc'] = roc_auc_score(y_true=y_true, y_score=y_score)
        metrics['aucpr'] = average_precision_score(y_true=y_true, y_score=y_score, pos_label=1)
        metrics['aucpr_normal'] = average_precision_score(y_true=y_true, y_score=y_score, pos_label=0)
